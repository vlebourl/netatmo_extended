...
SCHEMA_SERVICE_SETPERSONAWAY = vol.Schema({vol.Optional(ATTR_PERSON): cv.string,})
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    def _service_setpersonshome(service):
        ...
        _LOGGER.info("Set %s at home", persons)
    ...
    def _service_setpersonaway(service):
        ...
        if person_id is not None:
            camera_data.camera_data.set_persons_away(
                person_id=person_id, home_id=camera_data.camera_data.default_home_id
            )
    ...
    if camera_data.camera_data is not None:
